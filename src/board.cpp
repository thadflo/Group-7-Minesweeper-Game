...
constexpr bool in_3_by_3_range(uint8_t x, uint8_t y, uint8_t new_x, uint8_t new_y) {
	bool below = new_y <= min(y + 1, GRID_SIZE), above = new_y >= min(y - 1, y);
	bool left = new_x <= min(x + 1, GRID_SIZE), right = new_x >= min(x - 1, x);
...
void Board::update_tile_values(uint8_t x, uint8_t y) {
	...
	constexpr uint8_t max_value = GRID_SIZE - 1;
...
void Board::initialize(uint8_t bomb_count, uint8_t x, uint8_t y) {
	...
	while (bomb_count > 0) {
		uint8_t rand_x = rand() % GRID_SIZE;
		uint8_t rand_y = rand() % GRID_SIZE;
...
void Board::uncover_surrounding(uint8_t x, uint8_t y) {
	...
	const uint8_t end_y = min(GRID_SIZE - 1, y + 1);
	const uint8_t start_x = min(x - 1, x);
	const uint8_t end_x = min(GRID_SIZE - 1, x + 1);
